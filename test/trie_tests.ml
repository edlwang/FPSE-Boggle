...
let test_basic _ =
  (* This tests empty, get_child, is_endpoint *)
...
let test_insert _ =
  let trie =
    Trie.empty () |> Trie.insert ~word:"mason" |> Trie.insert ~word:"max"
  in
  ...
  assert_bool "failure" @@ is_some trie;
  assert_bool "failure"
  @@ is_some (Trie.get_child (Option.value trie ~default) 'x');
...
let test_is_word _ =
  ...
  assert_bool "failure" (not @@ Trie.is_word trie ~word:"bird")
